...
#=TrieTest.PrintTrie()
# 트라이 탐색
PrefixList = TrieTest.GetPrefixList('가격')
#=print(PrefixList)

# (단어, 빈도) 튜플 목록을 빈도순 정렬(sort)
PrefixList_Sort = sorted(PrefixList, key=lambda item:-item[1])#by high
#=print(PrefixList_Sort)
print(f'[빈도 순서]prefix : {len(PrefixList_Sort)}')
#=print(*PrefixList_Sort, sep='\n')
for i, Item in enumerate(PrefixList_Sort):
    print(f'{i+1}, {Item[0]} : {Item[1]}') # format: {('that', 13), ...}


'''
출력 결과:
=========================================
이 예제는 [KBS 9시 뉴스 16년치] 정렬된 사전 목록을 사용한다.

사전 순서상 앞에 가까이 있는 접두어 '가격'(위치:13599) 
사전 순서상 끝에 가까이 있는 접두어 '희망'(위치:99629)
=========================================
Dict Num: 413165
[빈도 순서]prefix : 77
1, 가격 : 7286
